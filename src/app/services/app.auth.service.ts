  ...
  public getRoles(): Observable<Array<string>> {
    if (this._decodedAccessToken !== null) {
      return new Observable<Array<string>>(observer => {
        if (this._decodedAccessToken.resource_access.timetracker.roles) {
          if (Array.isArray(this._decodedAccessToken.resource_access.timetracker.roles)) {
            const resultArr = this._decodedAccessToken.resource_access.timetracker.roles.map((r: string) => r.replace('ROLE_', ''));
            observer.next(resultArr);
          } else {
            observer.next([this._decodedAccessToken.resource_access.timetracker.roles.replace('ROLE_', '')]);
          }
        }
      });
    }
